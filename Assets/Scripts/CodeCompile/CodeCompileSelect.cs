...
public class CodeCompileSelect : MonoBehaviour
{
    public GameObject LoadingRunBtn;
    public GameObject LoadingNewBtn;
    public Text[] codeInput;
    public Image[] SelectBtnImage;    // Image ที่ใช้แสดงผลปุ่มที่เลือก
    public GameObject[] ChoiceBtn; // ปุ่มที่ใช้เลือกภาษา
    public Sprite imageSelect;  // Sprite ที่ใช้แสดงผลปุ่มที่เลือก
    public Sprite imageSelect2;  // Sprite ที่ใช้แสดงผลปุ่มที่เลือก
    public Sprite imageDontSelect;  // Sprite ที่ใช้แสดงผลปุ่มที่ไม่เลือก
    public Sprite imageClickChoice;  // Sprite ที่ใช้แสดงผลปุ่มทีเมื่อเลือกแล้ว
    ...
    private void Start()
    {
        ...
        {
            ChoiceBtn[i].SetActive(false); // ซ่อนปุ่มทั้งหมดในตอนเริ่มต้น
        }
    ...
    public IEnumerator OnCompileJavaAuto()
    {
        ...
        LoadingRunBtn.SetActive(false); 
        LoadingNewBtn.SetActive(false); 
    }
    ...
    {
        for (int i = 0; i < SelectBtnImage.Length; i++)
        {
            if (i == index)
            {
                if (string.IsNullOrEmpty(codeInput[i].text))
                {
                    // ยังไม่มีค่า
                    // ปุ่มที่กดอยู่ตอนนี้: โชว์ปุ่มและใช้ imageSelect
                    ChoiceBtn[i].SetActive(true);
                    SelectBtnImage[i].sprite = imageSelect;
                }
                else
                {
                    // มีค่าแล้ว: แสดงเป็น imageClickChoice แต่ไม่ต้องโชว์ปุ่ม
                    ChoiceBtn[i].SetActive(true);
                    SelectBtnImage[i].sprite = imageSelect2;
                }
            }
            else
            {
                if (string.IsNullOrEmpty(codeInput[i].text))
                {
                    // ยังไม่มีค่า: ซ่อนปุ่มและใช้ imageDontSelect
                    ChoiceBtn[i].SetActive(false);
                    SelectBtnImage[i].sprite = imageDontSelect;
                }
                else
                {
                    // มีค่าแล้ว: แสดงเป็น imageClickChoice แต่ไม่ต้องโชว์ปุ่ม
                    ChoiceBtn[i].SetActive(false);
                    SelectBtnImage[i].sprite = imageClickChoice;
                }
            }
        }
    }
    ...
    public Text choicetxt; // ตัวแปรเก็บค่าที่เลือก
    public void OnClickChoiceButton(Text choiceText)
    {
        choicetxt = choiceText; // เก็บค่าที่เลือก
    }

    public void OnClickChoiceButtonProcess(int index)
    {
        // เปลี่ยน Sprite ของปุ่มที่เลือก
        for (int i = 0; i < SelectBtnImage.Length; i++)
        {
            if (i == index)
            {
                ChoiceBtn[i].SetActive(false);
                SelectBtnImage[i].sprite = imageClickChoice;
                codeInput[i].text = choicetxt.text; // แสดงโค้ดที่เลือกใน TMP_Text
            }
            else
            {
                // ✅ เช็คว่ามี text อยู่ไหม
            if (string.IsNullOrEmpty(codeInput[i].text))
            {
                SelectBtnImage[i].sprite = imageDontSelect;
            }
    ...
    public IEnumerator OnCompileJavaLocal()
    {
        ...
        if (userInput.Any(string.IsNullOrEmpty))
        {
            ...
            UnityEngine.Debug.Log("One or more input fields are empty");
            yield break; 
        }
    ...
    public IEnumerator SendCodeJava(string code)
    {
        ...
        if (compileProcess.ExitCode != 0)
        {
            ...
            UnityEngine.Debug.Log($"Compilation Error: {error}");
